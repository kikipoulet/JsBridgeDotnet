...
function TodoList() {
  const todo = useService('TodoList');
  const [newTodo, setNewTodo] = useState('');

  const addTodo = async () => {
    if (newTodo.trim()) {
      await todo.Methods.Add(newTodo);
      setNewTodo('');
    }
  };

  const removeTodo = async (id) => await todo.Methods.Remove(id);

  const handleKeyPress = (e) => {
    if (e.key === 'Enter') addTodo();
  };

  if (todo._loading) return <div className="flex justify-center items-center h-full">Chargement...</div>;

  const todos = todo.Collections.Todos || [];
